...
var _import = function _import() {
  var mailparser = new MailParser();
  mailparser.on("end", parse);
  // fs.createReadStream(path.resolve('exports/readmill-likes-and-kindle-bookmarklet')).pipe(mailparser);
  // fs.createReadStream(path.resolve('exports/readmill/reading-data-attachment')).pipe(mailparser);
  // fs.createReadStream(path.resolve('exports/kindle/clippings-email')).pipe(mailparser);
  fs.createReadStream(path.resolve('exports/ibooks/email')).pipe(mailparser);
};